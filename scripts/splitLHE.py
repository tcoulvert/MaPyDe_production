...
inputfile = gzip.open(inputfilename, "rt")
...
for lineIndex, line in enumerate(inputfile):
    if '<event>' in line: 
        event_start_indices.append(lineIndex)
    if '</event>' in line: 
        event_end_indices.append(lineIndex)
    if len(event_start_indices) == 0: 
        ...
        if '<event>' in line and event_counter == maxNEvents: #open a new file and write header + write this line
            ...
            if fileIndex > 0:
                ...
                if fileIndex == index+1:
                    break
            #Open new file
            tempOutputFileName = outputfilename+".lhe"
